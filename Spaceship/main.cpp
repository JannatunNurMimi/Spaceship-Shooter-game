...
#define MENU_SCREEN 4 //made up of two screens used to specify the characteristics of runtime menu screens and the responses to be listed on that menu
...
GLint m_viewport[4]; //GLint=data types, set the viewport
GLint CI=0; //GLint=data types,
int x,y;
int i;
int randomStoneIndices[100]; //
int index;
int Score=0; //score
int alienLife=100; //Life of player
int GameLvl= 1;   //Game level
...
GLfloat a[][2]={0,-50, 70,-50, 70,70, -70,70}; //two dimentional array for 2D animation
GLfloat LightColor[][3]={1,1,0,   0,1,1,   0,1,0}; //RGB
...
void display(); //the function call
void StoneGenerate();  //generates stones
...
void SetDisplayMode(int modeToDisplay) {
		...
		case GAME_SCREEN:  glClearColor(0.0, 0.0, 0.0, 1.0);break;  //set game screen color
		case MENU_SCREEN :  glClearColor(1.0, 0.5, 0.0, 1.0);break;  //set menu screen color
	}
}
...
void DrawAlienBody() //draw alien body
{
	glColor4f(1.0f, 0.0f, 1.0f, 0.0f); //purple BODY color
	glBegin(GL_POLYGON); //draw polygon for body
	for(i=0;i<=8;i++)
    glVertex2fv(AlienBody[i]); //A pointer to an array of two elements. The elements are the x and y coordinates of a vertex. here array is AlienBody[i]
	glEnd(); //end

	glColor3f(0,0,0);			//BODY Outline color= black
	...
	glBegin(GL_LINES);                //draw line for Body effect
    glVertex2f(-13,11);
    glVertex2f(-15,9);
	glEnd();
}
void DrawAlienCollar() //draw alien collar
{
    glColor3f(1.0f, 0.5f, 0.0f); //Orange	COLLAR
	glBegin(GL_POLYGON);
	for(i=0;i<=20 ;i++)
...
void DrawAlienFace() //draw alien face
{
...
void DrawAlienBeak() //draw alien beak
{
...
void DrawAlienEyes() //draw alien eyes
{

	glColor3f(0.0f, 0.0f, 0.0f);  //Black

	glPushMatrix(); //save and restore the untranslated and unscaled coordinate system
	glRotated(-10,0,0,1); //
	glTranslated(-6,32.5,0);   //Left eye
	glScalef(2.5,4,0); //
	glutSolidSphere(1,20,30); //draws a shaded sphere centered at the origin
	glPopMatrix(); //save and restore the untranslated and unscaled coordinate system
...
void DrawSteeringWheel() //draw steering wheel
{
	glPushMatrix();
	glLineWidth(3);
...
void startScreenDisplay()  //start screen display
{
	glLineWidth(50);
...
void GameScreenDisplay() //game screen display
{
